...
BEFORE INSERT ON provider_trust_admissions
WHEN NEW.revision IS NOT COALESCE((SELECT MAX(revision)+1
        FROM provider_trust_admissions WHERE registration_id=NEW.registration_id),1)
    OR aios_trust_admission_matches_insert_v1(
        NEW.admission_id, NEW.registration_id, NEW.decision_id, NEW.revision,
        NEW.trust_status, NEW.authority_ref, NEW.admitted_at, NEW.receipt_json,
        COALESCE((SELECT admitted_at FROM provider_trust_admissions
                  WHERE registration_id=NEW.registration_id ORDER BY revision DESC LIMIT 1),
                 (SELECT registered_at FROM provider_registrations
                  WHERE registration_id=NEW.registration_id)),
        (SELECT trust_status FROM provider_registrations
         WHERE registration_id=NEW.registration_id),
        (SELECT state FROM provider_registrations
         WHERE registration_id=NEW.registration_id)) IS NOT 1
