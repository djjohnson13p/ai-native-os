...
impl<'de> Deserialize<'de> for ArtifactUri {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        {
            return Err(serde::de::Error::custom("invalid Artifact URI scheme"));
        }
        Ok(Self(value))
    }
}
...
impl TryFrom<ArtifactHandleWire> for ArtifactHandle {
    type Error = TaskManagerError;

    fn try_from(wire: ArtifactHandleWire) -> Result<Self> {
        validate_schema_string(&wire.artifact_id, 1, Some(512), "invalid Artifact ID")?;
        validate_schema_semantic_type(wire.semantic_type.as_deref())?;
        if wire.media_type.is_empty() {
            return Err(TaskManagerError::InvalidRecord(
                "invalid Artifact media type",
            ));
        }
        if let Some(format) = wire.format.as_deref() {
            validate_schema_string(format, 0, Some(160), "invalid Artifact format")?;
        }
        if let Some(hash) = wire.content_hash.as_ref() {
            if !matches!(hash.algorithm.as_str(), "sha256" | "sha512" | "blake3")
                || hash.value.is_empty()
            {
                return Err(TaskManagerError::InvalidRecord(
        ...
        if let Some(program_hash) = wire.origin.semantic_program_hash.as_deref() {
            let valid = program_hash.is_empty()
                || program_hash
                    .split_once(':')
            ...
            if !valid {
                return Err(TaskManagerError::InvalidRecord(
                    "invalid Artifact origin program hash",
                ));
            }
        }
        if let Some(expires_at) = wire
            .retention
            .as_ref()
            .and_then(|retention| retention.expires_at.as_deref())
        {
            parse_time(expires_at)?;
        }
        if let Some(verified_at) = wire
            .integrity
            .as_ref()
            .and_then(|integrity| integrity.verified_at.as_deref())
        {
            parse_time(verified_at)?;
        }
        if let Some(created_at) = wire.created_at.as_deref() {
            parse_time(created_at)?;
        }
        if !all_unique(&wire.labels) || wire.labels.iter().any(|label| label.chars().count() > 256)
        {
            return Err(TaskManagerError::InvalidRecord("invalid Artifact labels"));
        }
        Ok(Self {
...
impl<'de> Deserialize<'de> for ArtifactHandle {
    ...
    where
        D: serde::Deserializer<'de>,
    {
        Self::try_from(ArtifactHandleWire::deserialize(deserializer)?)
...
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct ArtifactOutputAllocationWire {
    ...
    state: ArtifactAllocationState,
    publication_id: Option<String>,
    published_artifact_id: Option<String>,
    created_at: String,
    expires_at: String,
    updated_at: Option<String>,
}

impl TryFrom<ArtifactOutputAllocationWire> for ArtifactOutputAllocation {
    type Error = TaskManagerError;

    fn try_from(wire: ArtifactOutputAllocationWire) -> Result<Self> {
        if wire.schema_version != SCHEMA_VERSION {
            return Err(TaskManagerError::InvalidRecord(
                "invalid Artifact allocation schema version",
            ));
        }
        validate_id(&wire.allocation_id, 256, "invalid Artifact allocation ID")?;
        validate_id(&wire.task_id, 256, "invalid Artifact allocation Task")?;
        validate_hash(&wire.semantic_program_hash)?;
        validate_id(&wire.node_id, 128, "invalid Artifact allocation node")?;
        validate_optional_bounded(
            wire.binding_id.as_deref(),
            256,
            "invalid Artifact allocation binding",
        )?;
        validate_optional_bounded(
            wire.attempt_id.as_deref(),
            256,
            "invalid Artifact allocation attempt",
        )?;
        validate_optional_bounded(
            wire.output_port.as_deref(),
            128,
            "invalid Artifact output port",
        )?;
        if let Some(semantic_type) = wire.expected_semantic_type.as_deref() {
            validate_optional_semantic_type(Some(semantic_type))?;
        }
        if wire.allowed_media_types.len() > 32
            || !all_unique(&wire.allowed_media_types)
            || wire
                .allowed_media_types
                .iter()
                .any(|media_type| media_type.is_empty() || media_type.chars().count() > 160)
        {
            return Err(TaskManagerError::InvalidRecord(
                "invalid Artifact allocation media types",
            ));
        }
        validate_optional_bounded(
            wire.publication_id.as_deref(),
            256,
            "invalid Artifact publication ID",
        )?;
        validate_optional_bounded(
            wire.published_artifact_id.as_deref(),
            512,
            "invalid published Artifact ID",
        )?;
        parse_time(&wire.created_at)?;
        parse_time(&wire.expires_at)?;
        if let Some(updated_at) = wire.updated_at.as_deref() {
            parse_time(updated_at)?;
        }
        Ok(Self {
            schema_version: wire.schema_version,
            allocation_id: wire.allocation_id,
            task_id: wire.task_id,
            semantic_program_hash: wire.semantic_program_hash,
            node_id: wire.node_id,
            binding_id: wire.binding_id,
            attempt_id: wire.attempt_id,
            output_port: wire.output_port,
            expected_semantic_type: wire.expected_semantic_type,
            allowed_media_types: wire.allowed_media_types,
            max_size_bytes: wire.max_size_bytes,
            sensitivity: wire.sensitivity,
            retention: wire.retention,
            state: wire.state,
            publication_id: wire.publication_id,
            published_artifact_id: wire.published_artifact_id,
            created_at: wire.created_at,
...
impl<'de> Deserialize<'de> for ArtifactOutputAllocation {
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        Self::try_from(ArtifactOutputAllocationWire::deserialize(deserializer)?)
            .map_err(serde::de::Error::custom)
    }
}
...
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct ArtifactPublicationResultWire {
    schema_version: String,
    ...
    blob_reused: Option<bool>,
    provenance_event_id: Option<String>,
    provenance_event_hash: Option<String>,
    resulted_at: String,
}

impl TryFrom<ArtifactPublicationResultWire> for ArtifactPublicationResult {
    type Error = TaskManagerError;

    fn try_from(wire: ArtifactPublicationResultWire) -> Result<Self> {
        validate_publication_result_wire(&wire)?;
        Ok(Self {
            schema_version: wire.schema_version,
            publication_id: wire.publication_id,
            allocation_id: wire.allocation_id,
            ...
            size_bytes: wire.size_bytes,
            content_hash: wire.content_hash,
            blob_reused: wire.blob_reused,
            provenance_event_id: wire.provenance_event_id,
            provenance_event_hash: wire.provenance_event_hash,
            resulted_at: wire.resulted_at,
        })
    }
}

impl<'de> Deserialize<'de> for ArtifactPublicationResult {
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        Self::try_from(ArtifactPublicationResultWire::deserialize(deserializer)?)
            .map_err(serde::de::Error::custom)
    }
}
...
impl Drop for ArtifactReader {
    fn drop(&mut self) {
        let Some(admission) = self.reader_admission.as_ref() else {
            return;
        };
        if let Ok(mut live) = self.live_reader_admissions.lock() {
            live.remove(&admission.operation_id);
        }
    }
}
...
impl TaskManager {
    ...
    pub(crate) fn publish_artifact_output(
        ...
        let finalized = (|| -> Result<ArtifactPublicationResult> {
            ...
            let current_allocation = load_allocation_row(&transaction, &request.allocation_id)?
                .ok_or(TaskManagerError::InvalidRecord(
                    "ARTIFACT_ALLOCATION_NOT_FOUND",
                ))?;
            if current_allocation != allocation
                || validate_publication_allocation(request, &current_allocation, &resulted_at)
                    .is_err()
            {
                drop(transaction);
                if !blob_reused {
            ...
            let effective_sensitivity = Sensitivity::parse(&current_allocation.sensitivity)?;
            let effective_retention = RetentionClass::parse(&current_allocation.retention)?;
            validate_bound_output_policy(
                &transaction,
                &OutputAllocationRequest {
                    schema_version: SCHEMA_VERSION.to_owned(),
                    allocation_id: request.allocation_id.clone(),
                    task_id: current_allocation.task_id.clone(),
                    semantic_program_hash: current_allocation.semantic_program_hash.clone(),
                    node_id: current_allocation.node_id.clone(),
                    binding_id: current_allocation.binding_id.clone(),
                    attempt_id: current_allocation.attempt_id.clone(),
                    output_port: None,
                    expected_semantic_type: current_allocation.expected_semantic_type.clone(),
                    allowed_media_types: current_allocation.allowed_media_types.clone(),
                    max_size_bytes: current_allocation.max_size_bytes,
                    sensitivity: effective_sensitivity.clone(),
                    retention: effective_retention.clone(),
                    expires_at: current_allocation.expires_at.clone(),
                },
            )?;
            if let Err(error) =
            ...
            params![artifact_id,artifact_uri.as_str(),request.semantic_type,request.media_type,request.format,to_i64(size)?,content_hash,effective_sensitivity.as_str(),effective_retention.as_str(),request.task_id,allocation.semantic_program_hash,allocation.node_id,allocation.binding_id,origin_provider_id,resulted_at,serde_json::to_string(&request.labels)?],
        )?;
            transaction.execute(
    ...
    pub fn scope_artifact_reads(
        ...
        for artifact_id in artifact_ids {
            ...
            let replay = replayable_reader_admission(
                &self.connection,
                task_id,
                execution,
                artifact_id,
            ...
            let grant_id = if recovering {
                replay
                    .map(|admission| admission.grant_admission.grant_id)
                    .ok_or(TaskManagerError::InvalidRecord("ARTIFACT_AUTHORITY_DENIED"))?
            } else if let Some(grant) = exact_operation_grant(
                &self.connection,
                task_id,
                execution,
                "artifact.read",
    ...
    #[allow(
        clippy::too_many_lines,
        reason = "keeps recovery replay and ordinary reader admission checks in one authority boundary"
    )]
    fn prepare_artifact_reader(
        ...
        let now = self.clock.now();
        if task_is_recovering(&self.connection, &scope.task_id)? {
            let execution = scope
                .authority
                .execution
                .as_ref()
            ...
            let grant_id = scope
                ...
                .get(artifact_id)
                .ok_or(TaskManagerError::InvalidRecord("ARTIFACT_AUTHORITY_DENIED"))?;
            if replayable_reader_admission_for_grant(
                &self.connection,
                &scope.task_id,
                execution,
                artifact_id,
    ...
    pub fn export_artifact<W: ArtifactExportWriter>(
        ...
        drop(reader);
        if exported_hash != intent.content_hash {
            self.mark_content_hash_failed(&intent.content_hash, "CORRUPT")?;
            self.finish_unknown_export_operation(
                &destination.operation_id,
                &destination.intent_json,
                &destination.task_id,
    ...
    pub(crate) fn migrate_legacy_keyed_import_receipts(&mut self) -> Result<()> {
        ...
        let mut damaged_tasks = BTreeSet::new();
        let transaction = self
            .connection
            .transaction_with_behavior(TransactionBehavior::Immediate)?;
        assert_manager_lease(&transaction, &self.lease_owner, self.lease_epoch)?;
        ...
        for row in events {
            ...
            let physical = hash_internal_file(&self.artifact_store_dir, &artifact.5);
            let damage = match physical {
                Ok((physical_size, physical_hash))
            ...
            if let Some(durability_state) = damage {
                transaction.execute(
                    ...
                    params![content_hash, durability_state, row.2],
                )?;
                transaction.execute(
            ...
            &mut staged
        {
                ...
                || terminal_allocation
            {
                    ...
                    pending_publication.as_ref()
                {
                    ...
                    append_publication_failure_evidence(
                        &transaction,
                        &request,
                        reason_code,
                        &reconciled_at,
    ...
    fn fail_pending_publication(
        ...
        assert_manager_lease(&transaction, &lease_owner, lease_epoch)?;
        ensure_task_is_not_recovering(&transaction, &request.task_id)?;
        append_publication_failure_evidence(&transaction, request, reason_code, &now)?;
        transaction.execute(
    ...
    fn authenticate_committed_publication(
        ...
        if !committed_publication_receipt_authenticates(
            &self.connection,
            &request.task_id,
            &request.publication_id,
        )? {
...
fn admit_prepared_artifact_reader(
    ...
    let recovering = task_is_recovering(connection, &scope.task_id)?;
    ...
    connection.execute(
        "UPDATE artifacts
         SET integrity_state='verified',integrity_verifier='artifact-store:sha256'
         WHERE artifact_id=?1",
        [artifact_id],
    )?;
    connection.execute(
...
#[derive(Clone, Copy)]
struct ReaderDeliveryContext<'a> {
    lease_owner: &'a str,
    lease_epoch: i64,
    task_id: &'a str,
    execution: &'a ExecutionAuthority,
    artifact_id: &'a str,
    operation_id: &'a str,
    admission: &'a GrantAdmission,
    clock: &'a Arc<dyn Clock>,
}

fn mark_reader_admission_delivered_in_transaction(
    transaction: &Transaction<'_>,
    context: &ReaderDeliveryContext<'_>,
    delivered_at: &str,
) -> Result<()> {
    ...
    let existing: ArtifactReaderAdmissionReceipt = serde_json::from_str(&pending_receipt)?;
    if existing.kind == "artifact-reader-admission-delivered" {
        if authenticate_reader_admission(
            transaction,
            context.operation_id,
            context.task_id,
    ...
    let appended = append_event(transaction, context.task_id, &event)?;
    ...
    let changed = transaction.execute(
        ...
        params![context.operation_id, delivered, pending_receipt],
    )?;
    if changed == 1 {
        Ok(())
    } else {
...
fn mark_reader_admission_delivered(
    ...
    mark_reader_admission_delivered_in_transaction(&transaction, context, &delivered_at)?;
...
struct ReaderFenceContext<'a> {
    lease_owner: &'a str,
    lease_epoch: i64,
    task_id: &'a str,
    authority: &'a ReadAuthority,
    artifact_id: &'a str,
    reader_admission: Option<&'a ReplayableReaderAdmission>,
    recovery_replay_revision: Option<i64>,
}
...
fn validate_reader_fence(reader: &ArtifactReader) -> Result<()> {
    ...
    validate_reader_fence_in_connection(
        ...
        &ReaderFenceContext {
            lease_owner: &reader.lease_owner,
            lease_epoch: reader.lease_epoch,
            task_id: &reader.task_id,
            authority: &reader.authority,
            artifact_id: &reader.artifact_id,
            reader_admission: reader.reader_admission.as_ref(),
            recovery_replay_revision: reader.recovery_replay_revision,
        },
        &reader.clock.now(),
    )
}

fn validate_reader_fence_in_connection(
    connection: &Connection,
    context: &ReaderFenceContext<'_>,
    now: &str,
) -> Result<()> {
    let owns_fence = connection.query_row(
        "SELECT EXISTS(SELECT 1 FROM task_manager_lease WHERE singleton_id=1 AND owner_id=?1 AND fence_epoch=?2)",
        params![context.lease_owner, context.lease_epoch],
        |row| row.get::<_, bool>(0),
    )?;
    if !owns_fence {
        return Err(TaskManagerError::InvalidRecord("ARTIFACT_AUTHORITY_DENIED"));
    }
    let recovery_revision = connection
        .query_row(
            "SELECT revision FROM tasks WHERE task_id=?1 AND state='RECOVERING'",
            [context.task_id],
            |row| row.get::<_, i64>(0),
        ...
        .optional()?;
    if let Some(recovery_revision) = recovery_revision {
        ...
        if !authenticate_reader_admission(
            ...
            &admission.grant_admission,
            now,
            context.recovery_replay_revision != Some(recovery_revision),
        )? {
            return Err(TaskManagerError::InvalidRecord("ARTIFACT_AUTHORITY_DENIED"));
        }
    }
    ensure_no_unknown_artifact_export(connection, context.task_id)?;
    ensure_artifact_integrity_durable(connection, context.artifact_id)?;
    match &context.authority.execution {
        Some(execution) => {
            let current =
                load_execution_authority(connection, context.task_id, &execution.binding_id, now)?;
            let admission = context
                .reader_admission
                .ok_or(TaskManagerError::InvalidRecord("ARTIFACT_AUTHORITY_DENIED"))?;
            if current != *execution
        ...
        None => {
            if capture_read_authority(connection, context.task_id, None, now)? != *context.authority
            {
                return Err(TaskManagerError::InvalidRecord("ARTIFACT_AUTHORITY_DENIED"));
            }
        }
...
fn validate_publication_result_wire(wire: &ArtifactPublicationResultWire) -> Result<()> {
    if wire.schema_version != SCHEMA_VERSION
        || !wire.reason_code.starts_with("ARTIFACT_")
        || wire.reason_code.len() <= "ARTIFACT_".len()
        || !wire
            .reason_code
            .bytes()
    ...
    {
        return Err(TaskManagerError::InvalidRecord(
            "invalid Artifact publication result",
        ));
    }
    ...
    validate_schema_string(
        ...
        1,
        Some(256),
        "invalid Artifact publication Task",
    )?;
    if let Some(message) = wire.message.as_deref() {
        validate_schema_string(message, 0, Some(4096), "invalid Artifact message")?;
    }
    if let Some(artifact_id) = wire.artifact_id.as_deref() {
        validate_schema_string(artifact_id, 0, Some(512), "invalid published Artifact ID")?;
    }
    validate_schema_semantic_type(wire.semantic_type.as_deref())?;
    if let Some(media_type) = wire.media_type.as_deref() {
        validate_schema_string(media_type, 0, Some(160), "invalid Artifact media type")?;
    }
    if let Some(event_id) = wire.provenance_event_id.as_deref() {
        validate_schema_string(
            event_id,
            0,
            Some(256),
            "invalid Artifact provenance event ID",
        )?;
    }
    parse_time(&wire.resulted_at)?;
    for hash in [
        wire.content_hash.as_deref(),
        wire.provenance_event_hash.as_deref(),
    ]
    .into_iter()
    .flatten()
    {
    ...
    if wire.artifact_uri.as_deref().is_some_and(|uri| {
        !uri.is_empty() && (!uri.starts_with("artifact://") || uri.len() == "artifact://".len())
    }) {
        return Err(TaskManagerError::InvalidRecord(
            "invalid Artifact publication URI",
        ));
    }
    if wire.published
        && (wire.artifact_id.as_deref().is_none_or(str::is_empty)
            || wire.artifact_uri.as_deref().is_none_or(|uri| {
                !uri.starts_with("artifact://") || uri.len() == "artifact://".len()
            })
            || wire.media_type.as_deref().is_none_or(str::is_empty)
            || wire.size_bytes.is_none()
            || wire.content_hash.as_deref().is_none_or(str::is_empty)
            || wire.blob_reused.is_none()
            || wire
                .provenance_event_id
                .as_deref()
    ...
    {
        return Err(TaskManagerError::InvalidRecord(
            "published Artifact result is incomplete",
        ));
    }
    Ok(())
}
...
fn publication_failure_event(
    ...
    Ok(json!({
        "schema_version": SCHEMA_VERSION,
        "event_id": event_id("artifact-publication-failed", &request.publication_id),
        "task_id": request.task_id,
        "event_type": "artifact.integrity-failed",
        "timestamp": resulted_at,
        "actor": {"kind":"system-service","id":"service:artifact-store"},
        "input_artifacts": [],
        "output_artifacts": [],
        ...
        "details": {
            ...
            "allocation_id": request.allocation_id,
            "request_digest": canonical_text_digest("artifact-publication-request-v1", &request_json),
...
fn append_publication_failure_evidence(
    transaction: &rusqlite::Transaction<'_>,
    request: &ArtifactPublicationRequest,
    reason_code: &str,
    resulted_at: &str,
) -> Result<()> {
    let event = publication_failure_event(request, reason_code, resulted_at)?;
    append_event(transaction, &request.task_id, &event)?;
    Ok(())
}

/// Authenticates the complete durable publication receipt without reading blob
/// bytes. Startup recovery uses this same proof as exact publication replay;
/// physical content integrity is assessed separately by the Artifact Store.
#[allow(
    clippy::too_many_lines,
    reason = "the durable publication, Artifact, lineage, and provenance proof is one security boundary"
)]
pub(crate) fn committed_publication_receipt_authenticates(
    connection: &Connection,
    task_id: &str,
    publication_id: &str,
) -> Result<bool> {
    let row = connection
        .query_row(
            "SELECT p.request_json,p.result_json,p.artifact_id,p.content_hash,p.committed_at,
                    a.allocation_id,a.state,a.publication_id,a.published_artifact_id,
                    a.task_id,a.semantic_program_hash,a.node_id,a.binding_id,a.attempt_id,
                    a.sensitivity,a.retention,
                    r.uri,r.semantic_type,r.media_type,r.format,r.size_bytes,r.content_hash,
                    r.sensitivity,r.retention_class,r.origin_kind,r.origin_task_id,
                    r.origin_program_hash,r.origin_node_id,r.origin_binding_id,
                    r.origin_provider_id,r.integrity_state,r.labels_json,
             ...
             JOIN artifact_blobs b ON b.content_hash=p.content_hash
             JOIN task_artifacts t ON t.task_id=p.task_id AND t.artifact_id=p.artifact_id
             LEFT JOIN provenance_events e
               ON e.task_id=p.task_id
              AND e.event_id=json_extract(p.result_json,'$.provenance_event_id')
             WHERE p.task_id=?1 AND p.publication_id=?2 AND p.state='COMMITTED'",
    ...
    let Ok(request) = serde_json::from_str::<ArtifactPublicationRequest>(&row.0) else {
        return Ok(false);
    };
    let Some(result_json) = row.1.as_deref() else {
        return Ok(false);
    };
    let Ok(result) = serde_json::from_str::<ArtifactPublicationResult>(result_json) else {
        return Ok(false);
    };
    if validate_publication_request(&request).is_err() || canonical_json(&request)? != row.0 {
        return Ok(false);
    }
    let Some(artifact_id_value) = row.2.as_deref() else {
        return Ok(false);
    };
    let Some(content_hash) = row.3.as_deref() else {
        return Ok(false);
    };
    let Some(committed_at) = row.4.as_deref() else {
        return Ok(false);
    };
    let size = u64::try_from(row.20).unwrap_or(u64::MAX);
    let blob_size = u64::try_from(row.32).unwrap_or(u64::MAX);
    let origin_provider_id = row
        .12
        .as_deref()
        .map(|binding_id| {
            connection.query_row(
                "SELECT provider_id FROM execution_bindings
             WHERE binding_id=?1 AND attempt_id=?2 AND task_id=?3",
                params![binding_id, row.13, task_id],
                |binding| binding.get::<_, String>(0),
            )
        })
        .transpose()?;
    let event = row
        ...
        .and_then(|value| serde_json::from_str::<serde_json::Value>(value).ok());
    let event_exact = event.as_ref().is_some_and(|event| {
        let historical_blob_reused_compatible = match event.pointer("/details/blob_reused") {
            None => true,
            Some(value) => value.as_bool() == result.blob_reused,
        };
        event
            .get("schema_version")
            .and_then(serde_json::Value::as_str)
            == Some(SCHEMA_VERSION)
            && event.get("event_id").and_then(serde_json::Value::as_str)
                == Some(event_id("artifact-created", artifact_id_value).as_str())
            && event.get("task_id").and_then(serde_json::Value::as_str) == Some(task_id)
            && event.get("event_type").and_then(serde_json::Value::as_str)
                == Some("artifact.created")
            && event.get("timestamp").and_then(serde_json::Value::as_str) == Some(committed_at)
            && event
                .pointer("/actor/kind")
                .and_then(serde_json::Value::as_str)
                == Some("system-service")
            ...
            && event
                .get("semantic_program_hash")
                .and_then(serde_json::Value::as_str)
                == Some(row.10.as_str())
            && event.get("execution_binding_id").is_some()
            && event
                .get("execution_binding_id")
                .and_then(serde_json::Value::as_str)
                == row.12.as_deref()
            && event.get("provider_id").is_some()
            && event.get("provider_id").and_then(serde_json::Value::as_str)
                == origin_provider_id.as_deref()
            && event
                .get("input_artifacts")
                .and_then(serde_json::Value::as_array)
                .is_some_and(|values| {
            ...
            && event
                ...
                .and_then(serde_json::Value::as_array)
                .is_some_and(|values| {
                    values.as_slice() == [serde_json::Value::String(artifact_id_value.to_owned())]
                })
    ...
    let lineage = {
        let mut statement = connection.prepare(
            "SELECT parent_artifact_id,relationship FROM artifact_lineage
             WHERE child_artifact_id=?1 ORDER BY parent_artifact_id,relationship",
        )?;
        statement
            .query_map([artifact_id_value], |lineage_row| {
                Ok((
                    lineage_row.get::<_, String>(0)?,
                    lineage_row.get::<_, String>(1)?,
                ))
    ...
    let expected_lineage = request
        ...
        .collect::<BTreeSet<_>>();
    let result_exact = result.schema_version == SCHEMA_VERSION
        && result.publication_id == publication_id
        && result.allocation_id == row.5
        && result.task_id == task_id
        && result.published
        && result.reason_code == "ARTIFACT_PUBLICATION_APPLIED"
        && result.message.is_none()
        && result.blob_reused.is_some()
        && artifact_id_value == artifact_id("publication", publication_id)
        && result.artifact_id.as_deref() == Some(artifact_id_value)
        && row.16 == ArtifactUri::new(artifact_id_value).as_str()
        && result.artifact_uri.as_deref() == Some(row.16.as_str())
        && result.semantic_type == row.17
        && result.media_type.as_deref() == Some(row.18.as_str())
        && result.size_bytes == Some(size)
        && result.content_hash.as_deref() == Some(content_hash)
    ...
    let materialized_exact = row.6 == "PUBLISHED"
        ...
        && row.9 == task_id
        && row.17 == request.semantic_type
        && row.18 == request.media_type
        && row.19 == request.format
        && row.21 == content_hash
        && row.22 == row.14
        && row.23 == row.15
        && row.24 == "task"
        && row.25.as_deref() == Some(task_id)
        && row.26.as_deref() == Some(row.10.as_str())
    ...
    let envelope_exact = row.37.as_deref() == Some(task_id)
        && row.38.as_deref() == Some("artifact.created")
        && row.39.as_deref() == Some(row.10.as_str())
        && row.40.as_deref() == Some(row.11.as_str())
        && row.41 == row.12
        && row.42 == origin_provider_id
        && row.43.as_deref() == Some("success");
    let provenance_exact = super::verify_provenance_through(connection, task_id, None)?;
    Ok(request.publication_id == publication_id
        && request.allocation_id == row.5
        && request.task_id == task_id
        && result_exact
        && materialized_exact
        && envelope_exact
...
fn authenticate_failed_publication(
    ...
    let expected_event = publication_failure_event(request, &result.reason_code, committed_at)?;
    let stored_event = connection
        .query_row(
            "SELECT event_json FROM provenance_events
             WHERE task_id=?1 AND event_id=?2 AND event_type='artifact.integrity-failed'",
            params![
                request.task_id,
                event_id("artifact-publication-failed", &request.publication_id)
            ],
            |row| row.get::<_, String>(0),
        )
        .optional()?
        .ok_or(TaskManagerError::InvalidRecord(
            "stored Artifact publication failure evidence is missing",
        ))?;
    if canonical_json(&expected_event)? != stored_event
        || !super::verify_provenance_through(connection, &request.task_id, None)?
    {
        return Err(TaskManagerError::InvalidRecord(
            "stored Artifact publication failure evidence is invalid",
        ));
    }
    Ok(result)
}

pub(crate) fn failed_expired_publication_receipt_authenticates(
    connection: &Connection,
    task_id: &str,
    publication_id: &str,
) -> Result<bool> {
    ...
    match authenticate_failed_publication(
        ...
        Ok(result) => Ok(result.reason_code == "ARTIFACT_ALLOCATION_EXPIRED"),
...
fn ensure_task_is_not_recovering(connection: &Connection, task_id: &str) -> Result<()> {
    let recovering = task_is_recovering(connection, task_id)?;
    if recovering {
...
fn task_is_recovering(connection: &Connection, task_id: &str) -> Result<bool> {
    connection
        .query_row(
...
pub(crate) fn event_id(kind: &str, identity: &str) -> String {
    let mut hasher = Sha256::new();
...
fn validate_schema_semantic_type(value: Option<&str>) -> Result<()> {
...
#[cfg(test)]
fn reader_delivery_before_transaction_step() -> Result<()> {
    READER_DELIVERY_BEFORE_TRANSACTION_TEST_HOOK.with(|hook| {
        if let Some(hook) = hook.borrow_mut().take() {
            hook()?;
        }
        Ok(())
...
#[allow(
    clippy::unnecessary_wraps,
    reason = "test race injection shares the serialized reader delivery boundary"
)]
fn reader_delivery_before_transaction_step() -> Result<()> {
    Ok(())
}
...
mod tests {
    ...
    struct CoordinatedFinalizeWriter {
        ...
        ordering: Arc<AtomicUsize>,
        effect_order: Arc<AtomicUsize>,
    }

    impl Write for CoordinatedFinalizeWriter {
        fn write(&mut self, bytes: &[u8]) -> std::io::Result<usize> {
            Ok(bytes.len())
        }
    ...
    impl ArtifactExportWriter for CoordinatedFinalizeWriter {
        fn finalize(&mut self) -> std::io::Result<()> {
            self.finalize_ready.wait();
            self.lock_conflict_observed.wait();
            self.effect_order.store(
                self.ordering.fetch_add(1, Ordering::SeqCst) + 1,
                Ordering::SeqCst,
            );
            Ok(())
        }
    ...
    fn authenticate_running_fixture_history(manager: &mut TaskManager) {
        ...
        .enumerate()
        {
            let previous_revision = i64::try_from(index + 1).unwrap();
            ...
            let event = json!({
                ...
                "committed_mutation": {
                    "active_plan": null,
                    "active_step_ids": if index == 0 { json!(["compose_report"]) } else { serde_json::Value::Null },
                    "waiting_on": null,
                    "failure": null,
                    "recovery": null
                },
            ...
            transaction
                .execute(
                    ...
                    params![
                        ...
                        json!({"code":"STATE_CHANGE_REQUESTED","message":null,"provenance_event_id":appended.event_id}).to_string()
                    ],
                )
                .unwrap();
            from = to;
    ...
    fn assert_json_matches_schema(schema_name: &str, instance: &serde_json::Value) {
        let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
        let schema: serde_json::Value =
            serde_json::from_slice(&std::fs::read(root.join("specs").join(schema_name)).unwrap())
                .unwrap();
    ...
    fn valid_artifact_handle_json(uri: &str) -> serde_json::Value {
        json!({
            ...
            "format": "text",
            "size_bytes": 4,
            "content_hash": {"algorithm":"sha256","value":"abcd"},
            "origin": {
                "kind": "task",
                "task_id": "T-artifact",
                "semantic_program_hash": "sha256:abcd",
                "step_id": "compose_report",
                "execution_binding_id": "binding:test",
                "provider_id": "provider:test"
            },
            "sensitivity": "local",
            "retention": {"class":"task","expires_at":"2026-09-20T00:00:00Z"},
            "integrity": {"state":"verified","verified_at":"2026-09-19T00:00:00Z","verifier":"test"},
            "labels": ["fixture"],
            "created_at": "2026-09-19T00:00:00Z"
        })
    }

    fn valid_publication_result_json() -> serde_json::Value {
        json!({
            "schema_version": "0.1",
            ...
            "message": null,
            "artifact_id": "artifact:test",
            "artifact_uri": "artifact://artifact:test",
            "semantic_type": "artifact.report@1",
            "media_type": "text/plain",
            "size_bytes": 4,
            "content_hash": format!("sha256:{}", "a".repeat(64)),
            "blob_reused": false,
            "provenance_event_id": "event:test",
            "provenance_event_hash": format!("sha256:{}", "b".repeat(64)),
            "resulted_at": "2026-09-19T00:00:00Z"
        })
    }

    #[test]
    fn artifact_handle_deserialization_matches_published_contract() {
        for scheme in ["artifact", "data", "stream", "model", "secret"] {
            let value = valid_artifact_handle_json(&format!("{scheme}://fixture"));
            serde_json::from_value::<ArtifactHandle>(value).unwrap();
        }
        for uri in [
            "",
            "file:///etc/passwd",
            ...
            assert!(
                serde_json::from_value::<ArtifactHandle>(valid_artifact_handle_json(uri)).is_err()
            );
        }

        let mut invalid = valid_artifact_handle_json("artifact://fixture");
        invalid["created_at"] = json!("not-a-date");
        assert!(serde_json::from_value::<ArtifactHandle>(invalid).is_err());

        let mut invalid = valid_artifact_handle_json("artifact://fixture");
        invalid["labels"] = json!(["duplicate", "duplicate"]);
        assert!(serde_json::from_value::<ArtifactHandle>(invalid).is_err());

        let mut invalid = valid_artifact_handle_json("artifact://fixture");
        invalid["labels"] = json!(["x".repeat(257)]);
        assert!(serde_json::from_value::<ArtifactHandle>(invalid).is_err());

        let mut many_labels = valid_artifact_handle_json("artifact://fixture");
        many_labels["labels"] = serde_json::Value::Array(
            (0..257)
                .map(|index| json!(format!("label-{index}")))
                .collect(),
        );
        serde_json::from_value::<ArtifactHandle>(many_labels).unwrap();

        for (algorithm, value) in [("md5", "abcd"), ("sha256", "")] {
            let mut invalid = valid_artifact_handle_json("artifact://fixture");
            invalid["content_hash"] = json!({"algorithm":algorithm,"value":value});
            assert!(serde_json::from_value::<ArtifactHandle>(invalid).is_err());
        }

        let mut schema_boundaries = valid_artifact_handle_json("artifact://fixture");
        schema_boundaries["format"] = json!("");
        schema_boundaries["semantic_type"] = json!(format!("{}@1", "a".repeat(300)));
        assert_json_matches_schema("artifact-handle.schema.json", &schema_boundaries);
        serde_json::from_value::<ArtifactHandle>(schema_boundaries).unwrap();
    }

    #[test]
    fn publication_result_deserialization_enforces_conditional_success_contract() {
        serde_json::from_value::<ArtifactPublicationResult>(valid_publication_result_json())
            .unwrap();
        for field in [
            "artifact_id",
            "artifact_uri",
            ...
            let mut missing = valid_publication_result_json();
            missing.as_object_mut().unwrap().remove(field);
            assert!(
                serde_json::from_value::<ArtifactPublicationResult>(missing).is_err(),
                "published result accepted missing {field}"
            );
            let mut null = valid_publication_result_json();
            null[field] = serde_json::Value::Null;
            assert!(
                serde_json::from_value::<ArtifactPublicationResult>(null).is_err(),
                "published result accepted null {field}"
            );
        }

        for (field, value) in [
            ("artifact_uri", json!("file:///tmp/forged")),
            ("content_hash", json!("")),
            ("content_hash", json!("sha256:abcd")),
            ("provenance_event_hash", json!("")),
            ("provenance_event_hash", json!("sha256:ABCD")),
        ] {
            let mut invalid = valid_publication_result_json();
            invalid[field] = value;
            assert!(serde_json::from_value::<ArtifactPublicationResult>(invalid).is_err());
        }

        let mut failed = valid_publication_result_json();
        failed["published"] = json!(false);
        for field in [
            "artifact_id",
            "artifact_uri",
            "media_type",
            ...
            failed.as_object_mut().unwrap().remove(field);
        }
        serde_json::from_value::<ArtifactPublicationResult>(failed).unwrap();

        let mut schema_boundaries = valid_publication_result_json();
        schema_boundaries["published"] = json!(false);
        schema_boundaries["message"] = json!("");
        schema_boundaries["artifact_id"] = json!("");
        schema_boundaries["artifact_uri"] = json!(format!("artifact://{}", "a".repeat(2_048)));
        schema_boundaries["semantic_type"] = json!(format!("{}@1", "a".repeat(300)));
        schema_boundaries["media_type"] = json!("");
        schema_boundaries["content_hash"] = json!("");
        schema_boundaries["provenance_event_id"] = json!("");
        schema_boundaries["provenance_event_hash"] = json!("");
        assert_json_matches_schema(
            "artifact-publication-result.schema.json",
            &schema_boundaries,
        );
        serde_json::from_value::<ArtifactPublicationResult>(schema_boundaries).unwrap();

        let mut unknown = valid_publication_result_json();
        unknown["unexpected"] = json!(true);
        assert!(serde_json::from_value::<ArtifactPublicationResult>(unknown).is_err());
    }

    #[test]
    fn provider_reader_bounds_each_staging_allocation_and_consumes_one_grant_use() {
        let temp = TempDir::new().unwrap();
        let mut manager = manager(&temp);
        let content = vec![b'x'; COPY_BUFFER_SIZE + 1];
        let mut import = import_request();
        import.max_size_bytes = Some((COPY_BUFFER_SIZE + 1) as u64);
        let artifact = manager
            .import_artifact(&import, &mut Cursor::new(content.as_slice()))
            .unwrap();
        ...
        let mut reader = manager
            ...
            .unwrap();
        let mut destination = vec![0_u8; COPY_BUFFER_SIZE * 2];
        assert_eq!(reader.read(&mut destination).unwrap(), COPY_BUFFER_SIZE);
        assert_eq!(
            reader.read(&mut destination[COPY_BUFFER_SIZE..]).unwrap(),
            1
        );
        assert_eq!(&destination[..content.len()], content.as_slice());
        assert_eq!(
            manager
                .connection
                .query_row(
                    "SELECT uses_consumed FROM authority_grants WHERE grant_id='grant-bounded-reader-0'",
                    [],
                    |row| row.get::<_, i64>(0),
                )
                .unwrap(),
    ...
    fn writer_admission_database_error_removes_staging_and_preserves_retry() {
        ...
        manager
            .connection
            .execute_batch(
                "CREATE TRIGGER reject_writer_admission
                 BEFORE UPDATE OF state ON artifact_output_allocations
                 WHEN OLD.allocation_id='alloc-admission-cleanup'
                   AND OLD.state='ALLOCATED' AND NEW.state='WRITING'
                 BEGIN SELECT RAISE(ABORT, 'injected writer admission failure'); END;",
            )
            .unwrap();
        let session = session_for_allocation(&manager, allocation_id);
        assert!(matches!(
        ...
        assert_eq!(
            manager
                .connection
                .query_row(
                    "SELECT uses_consumed FROM authority_grants WHERE grant_id='grant-admission-cleanup-0'",
                    [],
                    |row| row.get::<_, i64>(0),
                )
                .unwrap(),
    ...
    #[test]
    fn export_hashes_the_retained_reader_bytes_and_marks_a_mismatch_unknown() {
        let temp = TempDir::new().unwrap();
        let mut manager = manager(&temp);
        let artifact = manager
            .import_artifact(
                &import_request(),
                &mut Cursor::new(b"authenticated export bytes".as_slice()),
            )
            .unwrap();
        let storage_ref = manager
            ...
            .unwrap();
        let physical = resolve_internal_ref(&manager.artifact_store_root, &storage_ref).unwrap();
        let scope = manager
        ...
        EXPORT_COPY_TEST_HOOK.with(|hook| {
            *hook.borrow_mut() = Some(Box::new(move || {
                std::fs::write(physical, b"forged export bytes......")?;
                Ok(())
            }));
        });
            ...
            manager
                ...
                .query_row(
                    "SELECT state || ':' || outcome_certainty FROM operations
                     WHERE operation_id='export-byte-hash-mismatch'",
                    [],
                    |row| row.get::<_, String>(0),
                )
                .unwrap(),
    ...
    fn startup_terminalizes_publication_reserved_before_allocation_link() {
        ...
        assert_eq!(canonical_json(&result).unwrap(), stored_result_json);
        {
            ...
            let resolution = crate::resolved_publication_recovery_subject(
                &transaction,
                "T-artifact",
                &request.publication_id,
            )
            .unwrap()
            .unwrap();
            assert_eq!(resolution.certainty, "FAILED_NO_EFFECT");
            assert_eq!(resolution.safe_action, "CLEAN_STAGING");
        }
        let seal = seal_ref(&staging_ref);
        for residue in [staging_ref.clone(), seal.clone(), format!("{seal}.pending")] {
    ...
    fn expired_publication_receipts_reject_paired_cross_publication_substitution() {
        ...
        let mut manager = manager(&temp);
        let requests = [
            publication("pub-expired-a", "alloc-expired-a"),
            publication("pub-expired-b", "alloc-expired-b"),
        ];
        ...
        let copied_evidence = manager
            .connection
            .query_row(
                "SELECT request_json,result_json,committed_at
                 FROM artifact_publications WHERE publication_id=?1",
                [&requests[1].publication_id],
                |row| {
                    Ok((
                        row.get::<_, String>(0)?,
                        row.get::<_, String>(1)?,
        ...
        manager
            ...
            .execute(
                "UPDATE artifact_publications
                 SET request_json=?2,result_json=?3,committed_at=?4
    ...
    #[test]
    fn publication_expiry_after_blob_placement_removes_unadopted_blob() {
        let temp = TempDir::new().unwrap();
        let mut manager = manager(&temp);
        let request = allocation("alloc-publication-expiry-race");
        manager.allocate_artifact_output(&request).unwrap();
        write_output(
            &mut manager,
            &request.allocation_id,
            b"post-placement expiry race",
        );
        let staging_ref = load_allocation_row(&manager.connection, &request.allocation_id)
            .unwrap()
            .unwrap()
            .staging_ref
            .unwrap();
        let publication = publication("pub-publication-expiry-race", &request.allocation_id);
        let database = temp.path().join("task-manager.sqlite");
        let allocation_id = request.allocation_id.clone();
        PUBLICATION_COMMIT_TEST_HOOK.with(|hook| {
            *hook.borrow_mut() = Some(Box::new(move || {
                Connection::open(database)?.execute(
                    "UPDATE artifact_output_allocations SET expires_at='2026-09-19T21:59:59Z'
                     WHERE allocation_id=?1",
                    [&allocation_id],
                )?;
                Ok(())
    ...
    fn final_publication_uses_time_sampled_after_serialized_commit_hook() {
        ...
        let request = allocation("alloc-publication-clock-race");
        manager.allocate_artifact_output(&request).unwrap();
        write_output(
            &mut manager,
            &request.allocation_id,
            b"clock advances before final lock",
        );
        let publication = publication("pub-publication-clock-race", &request.allocation_id);
        let advanced = Arc::clone(&now);
        PUBLICATION_COMMIT_TEST_HOOK.with(|hook| {
            *hook.borrow_mut() = Some(Box::new(move || {
                *advanced.lock().unwrap() = "2026-09-19T23:00:01Z".to_owned();
                Ok(())
            }));
        });
        ...
        assert_eq!(
            manager
                .connection
                .query_row(
                    "SELECT expires_at FROM artifact_output_allocations WHERE allocation_id=?1",
                    [&request.allocation_id],
                    |row| row.get::<_, String>(0),
                )
                .unwrap(),
    ...
    #[test]
    fn publication_commit_hook_failure_removes_fresh_unreferenced_blob() {
        let temp = TempDir::new().unwrap();
        let mut manager = manager(&temp);
        let request = allocation("alloc-publication-hook-failure");
        manager.allocate_artifact_output(&request).unwrap();
        write_output(
            &mut manager,
            &request.allocation_id,
            b"fresh blob must not become an orphan",
        );
        let publication = publication("pub-publication-hook-failure", &request.allocation_id);
        PUBLICATION_COMMIT_TEST_HOOK.with(|hook| {
            *hook.borrow_mut() = Some(Box::new(|| {
                Err(TaskManagerError::Io(std::io::Error::other(
                    "injected pre-transaction publication failure",
                )))
            }));
        });
    ...
    #[test]
    fn destination_finalize_holds_serialized_authority_fence_until_effect_completes() {
        let temp = TempDir::new().unwrap();
        let database = temp.path().join("task-manager.sqlite");
        let mut manager = manager(&temp);
        let artifact = manager
            .import_artifact(
                &import_request(),
                &mut Cursor::new(b"finalize under authority fence".as_slice()),
            )
            .unwrap();
        let scope = manager
            .scope_owned_artifact_reads("T-artifact", std::slice::from_ref(&artifact.artifact_id))
            .unwrap();
        let finalize_ready = Arc::new(Barrier::new(2));
        let lock_conflict_observed = Arc::new(Barrier::new(2));
        let conflict_count = Arc::new(AtomicUsize::new(0));
        let ordering = Arc::new(AtomicUsize::new(0));
        let effect_order = Arc::new(AtomicUsize::new(0));
        let revocation_order = Arc::new(AtomicUsize::new(0));
        let mut destination = manager
            .issue_owned_artifact_export_destination(
                &scope,
                "export-finalize-serialized-fence",
                &artifact.artifact_id,
                "user-selected-file",
                1_024,
                deferred(CoordinatedFinalizeWriter {
                    finalize_ready: Arc::clone(&finalize_ready),
                    lock_conflict_observed: Arc::clone(&lock_conflict_observed),
                    ordering: Arc::clone(&ordering),
                    effect_order: Arc::clone(&effect_order),
                }),
            )
            .unwrap();
        let revoker_ordering = Arc::clone(&ordering);
        let recorded_revocation = Arc::clone(&revocation_order);
        let recorded_conflict = Arc::clone(&conflict_count);
        let revoker = std::thread::spawn(move || {
            ...
            let conflict = connection
                .execute(
                    "UPDATE tasks SET principal_id='user:revoked-during-finalize'
                     WHERE task_id='T-artifact'",
                    [],
                )
                .unwrap_err();
            ...
            recorded_conflict.fetch_add(1, Ordering::SeqCst);
            lock_conflict_observed.wait();
            connection
            ...
            connection
                ...
                .unwrap();
            recorded_revocation.store(
                revoker_ordering.fetch_add(1, Ordering::SeqCst) + 1,
                Ordering::SeqCst,
            );
        });
        ...
        assert_eq!(
            manager
                .export_artifact(&scope, &artifact.artifact_id, &mut destination)
                .unwrap(),
            b"finalize under authority fence".len() as u64
        );
        revoker.join().unwrap();
        ...
        assert!(matches!(
            manager.issue_owned_artifact_export_destination(
                &scope,
                "export-after-finalize-revocation",
                &artifact.artifact_id,
                "user-selected-file",
                1_024,
                move || {
    ...
    fn revocation_after_blob_placement_is_caught_by_metadata_commit_fence() {
        ...
        let revoked_grant = grant_id.clone();
        PUBLICATION_COMMIT_TEST_HOOK.with(|hook| {
            *hook.borrow_mut() = Some(Box::new(move || {
                Connection::open(database)?.execute(
                    "UPDATE authority_grants
                     SET state='REVOKED',revoked_at='2026-09-19T22:00:00Z'
                     WHERE grant_id=?1",
                    [&revoked_grant],
                )?;
                Ok(())
        ...
        assert!(
            !blob.exists(),
            "rejected publication retained an unadopted placed blob"
        );
        assert_eq!(
            manager
                .connection
    ...
    fn machine_contract_dtos_reject_unknown_json_fields() {
        ...
        let allocation_response = serde_json::to_value(
            manager
                .allocate_artifact_output(&self::allocation("alloc-response-closed-dto"))
                .unwrap(),
        )
        .unwrap();
        let publication =
        ...
        let mut allocation_response = allocation_response;
        allocation_response
            .as_object_mut()
            .unwrap()
            .insert("unexpected".to_owned(), json!(true));
        assert!(serde_json::from_value::<ArtifactOutputAllocation>(allocation_response).is_err());
    ...
    fn committed_replay_authenticates_receipt_rows_and_blob_bytes() {
        ...
        let artifact_id = original.artifact_id.as_deref().unwrap();
        let scope = manager
            .scope_owned_artifact_reads("T-artifact", &[artifact_id.to_owned()])
            .unwrap();
        let mut reader = manager.open_artifact_reader(&scope, artifact_id).unwrap();
        let mut bytes = Vec::new();
        reader.read_to_end(&mut bytes).unwrap();
        assert_eq!(bytes, b"replay");
        assert_eq!(manager.publish_artifact_output(&request).unwrap(), original);

        let immutable_metadata = manager
            .connection
            .query_row(
                "SELECT expires_at,integrity_verified_at,integrity_verifier,created_at
                 FROM artifacts WHERE artifact_id=?1",
                [artifact_id],
                |row| {
                    Ok((
            ...
            .unwrap();
        for (column, forged, original_value) in [
            (
                "expires_at",
                "2027-01-01T00:00:00Z",
                immutable_metadata.0.as_deref(),
            ),
            (
                "integrity_verified_at",
                "2027-01-02T00:00:00Z",
                immutable_metadata.1.as_deref(),
            ),
            (
                "integrity_verifier",
                "forged:verifier",
                immutable_metadata.2.as_deref(),
            ),
            (
                "created_at",
                "2027-01-03T00:00:00Z",
                Some(immutable_metadata.3.as_str()),
            ),
        ] {
            manager
                .connection
                .execute(
                    &format!("UPDATE artifacts SET {column}=?1 WHERE artifact_id=?2"),
                    params![forged, artifact_id],
                )
                .unwrap();
            let rejected = manager.publish_artifact_output(&request).unwrap();
            assert!(!rejected.published, "replay accepted forged {column}");
            assert_eq!(rejected.reason_code, "ARTIFACT_INTEGRITY_FAILED");
            manager
                .connection
                .execute(
                    &format!("UPDATE artifacts SET {column}=?1 WHERE artifact_id=?2"),
                    params![original_value, artifact_id],
                )
                .unwrap();
            assert_eq!(manager.publish_artifact_output(&request).unwrap(), original);
        }
    ...
    #[test]
    fn startup_does_not_launder_forged_publication_verification_time() {
        let temp = TempDir::new().unwrap();
        let database = temp.path().join("task-manager.sqlite");
        let mut manager = manager(&temp);
        manager
            .allocate_artifact_output(&allocation("alloc-legacy-verification"))
            .unwrap();
        write_output(
            &mut manager,
            "alloc-legacy-verification",
            b"legacy verified",
        );
        let request = publication("pub-legacy-verification", "alloc-legacy-verification");
        let original = manager.publish_artifact_output(&request).unwrap();
        let artifact_id = original.artifact_id.as_deref().unwrap();
        manager
            .connection
            .execute(
                "UPDATE artifacts SET integrity_verified_at='2026-09-19T23:00:00Z'
                 WHERE artifact_id=?1",
                [artifact_id],
            )
            .unwrap();
        manager
            .connection
            .execute(
                "UPDATE artifact_blobs SET verified_at='2026-09-19T23:00:00Z'
                 WHERE content_hash=?1",
                [original.content_hash.as_deref().unwrap()],
            )
            .unwrap();
        ...
        let replay = reopened.publish_artifact_output(&request);
    ...
    fn output_allocation_wire_defaults_omitted_allowed_media_types() {
        ...
        for (name, pointer, replacement) in [
            ...
            ("created-at", "/created_at", json!("not-rfc3339")),
            ("expires-at", "/expires_at", json!("not-rfc3339")),
        ] {
            let mut invalid = canonical.clone();
            *invalid.pointer_mut(pointer).unwrap() = replacement;
            assert!(
                serde_json::from_value::<ArtifactOutputAllocation>(invalid).is_err(),
                "{name}"
            );
        }
        ...
        unknown
            ...
            .unwrap()
            .insert("unexpected".to_owned(), json!(true));
        assert!(serde_json::from_value::<ArtifactOutputAllocation>(unknown).is_err());
    }
    ...
    fn keyed_import_replays_after_lost_commit_response_without_reading_source() {
        ...
        new_request.import_id = Some("import:new-during-recovery".to_owned());
        assert!(matches!(
            manager.import_artifact(&new_request, &mut PanicReader),
            Err(TaskManagerError::InvalidRecord("ARTIFACT_AUTHORITY_DENIED"))
        ));
    }
    ...
    fn damaged_legacy_keyed_import_is_quarantined_into_recovery_instead_of_fabricated() {
        ...
        let mut manager = manager(&temp);
        let mut request = import_request();
        request.import_id = Some("import:damaged-legacy".to_owned());
        let artifact = manager
            .import_artifact(
                &request,
    ...
    #[test]
    fn writer_admission_waits_for_durable_staging_entry_before_consuming_grant() {
        let temp = TempDir::new().unwrap();
        let mut manager = manager(&temp);
        let allocation_id = "alloc-staging-durability";
        let (binding_id, attempt_id) = install_one_shot_binding(
            &manager,
            "staging-durability",
            &[],
        ...
        assert_eq!(
            manager
                ...
                .query_row(
                    "SELECT uses_consumed FROM authority_grants
                     WHERE grant_id='grant-staging-durability-0'",
                    [],
                    |row| row.get::<_, i64>(0),
                )
                .unwrap(),
    ...
    fn reader_admission_response_loss_replays_once_without_second_grant_use() {
        ...
        let mut reopened = TaskManager::open_with_clock(&database, Box::new(FixedClock)).unwrap();
        assert_eq!(
            reopened.get_task("T-artifact").unwrap().unwrap().state,
            crate::TaskState::Recovering
        );
        let session = reopened
            .issue_provider_artifact_session("T-artifact", &binding_id)
            .unwrap();
    ...
    #[test]
    fn delivered_provider_reader_cannot_continue_after_task_enters_recovery() {
        let temp = TempDir::new().unwrap();
        let mut manager = manager(&temp);
        let artifact = manager
            .import_artifact(
                &import_request(),
                &mut Cursor::new(b"delivered before recovery".as_slice()),
            )
            .unwrap();
        let (binding_id, _) = install_one_shot_binding(
            &manager,
            "delivered-before-recovery",
            std::slice::from_ref(&artifact.artifact_id),
            &[("artifact.read", "artifact", &artifact.artifact_id)],
        );
        authenticate_running_fixture_history(&mut manager);
        ...
        manager
            ...
            .execute(
                "INSERT INTO operations(
                    ...
                    'operation-reader-recovery','T-artifact',?1,'compose_report','NETWORK',
                    'UNKNOWN','OUTCOME_UNKNOWN','2026-09-19T22:00:00Z'
                 )",
                [allocation("unused").semantic_program_hash],
            )
            .unwrap();
        manager.reconcile_live_execution("T-artifact").unwrap();
        assert_eq!(
            reader.read(&mut byte).unwrap_err().kind(),
            std::io::ErrorKind::PermissionDenied
        );
    ...
    #[test]
    fn unread_reader_drop_seek_and_zero_length_read_preserve_exact_pending_replay() {
        let temp = TempDir::new().unwrap();
        let mut manager = manager(&temp);
        let artifact = manager
            .import_artifact(
                &import_request(),
                &mut Cursor::new(b"pending reader bytes".as_slice()),
            )
            .unwrap();
        let (binding_id, _) = install_one_shot_binding(
            &manager,
        ...
        let operation_id = first
            ...
            .operation_id
            .clone();
        assert_eq!(first.seek(SeekFrom::Start(0)).unwrap(), 0);
        ...
        let mut replayed = manager
            ...
            .unwrap();
        assert_eq!(
            replayed.reader_admission.as_ref().unwrap().operation_id,
            operation_id
        );
        let mut bytes = Vec::new();
        replayed.read_to_end(&mut bytes).unwrap();
        assert_eq!(bytes, b"pending reader bytes");
        assert_eq!(
            manager
                .connection
                .query_row(
                    "SELECT uses_consumed FROM authority_grants
                     WHERE grant_id='grant-pending-reader-drop-0'",
                    [],
                    |row| row.get::<_, i64>(0),
                )
                .unwrap(),
    ...
    #[test]
    fn retained_reader_rechecks_database_file_identity_before_first_byte() {
        let temp = TempDir::new().unwrap();
        let mut manager = manager(&temp);
        let artifact = manager
            .import_artifact(
                &import_request(),
                &mut Cursor::new(b"database identity fenced bytes".as_slice()),
            )
            .unwrap();
        let scope = manager
            .scope_owned_artifact_reads("T-artifact", std::slice::from_ref(&artifact.artifact_id))
        ...
        reader.database_identity =
            Some(crate::store_identity(&replacement, &replacement_file).unwrap());
    ...
    #[test]
    fn export_delivers_reader_before_arming_any_destination_effect() {
        let temp = TempDir::new().unwrap();
        let mut manager = manager(&temp);
        let artifact = manager
            .import_artifact(
                &import_request(),
                &mut Cursor::new(b"delivery before export arming".as_slice()),
            )
            .unwrap();
        let (binding_id, _) = install_one_shot_binding(
            &manager,
            "export-delivery-first",
            std::slice::from_ref(&artifact.artifact_id),
            &[
                ("artifact.read", "artifact", &artifact.artifact_id),
                ("data.egress", "destination", "user-selected-file"),
        ...
        READER_DELIVERY_COMMIT_TEST_HOOK.with(|hook| {
            *hook.borrow_mut() = Some(Box::new(|| {
                Err(TaskManagerError::Io(std::io::Error::other(
                    "injected reader delivery rollback before export arming",
                )))
            }));
        });
    ...
    fn mutate_reader_delivery_boundary(
        ...
        match scenario {
            ...
            "lease" => {
                connection.execute(
                    "UPDATE task_manager_lease SET owner_id='forged-owner',fence_epoch=fence_epoch+1
                     WHERE singleton_id=1",
                    [],
                )?;
            }
            "supersession" => {
                connection.execute_batch(&format!(
                    "INSERT INTO execution_bindings(binding_id,attempt_id,task_id,semantic_program_hash,registry_snapshot_id,ir_version,node_id,capability,provider_id,provider_version,attempt,policy_decision_refs_json,grant_refs_json,execution_profile_ref,placement_json,binding_json,created_at)
                     VALUES ('binding-delivery-later','attempt-delivery-later','T-artifact','{program_hash}','{registry_id}','0.1','compose_report','document.compose','provider:sequential','1',2,'[]','[]','profile:test','{{}}','{{}}','2026-09-19T00:01:00Z');
                     ...
                     VALUES ('attempt-delivery-later','T-artifact','{program_hash}','{registry_id}','compose_report','binding-delivery-later',2,1,'RUNNING','[]','[]','2026-09-19T00:01:00Z','2026-09-19T00:01:00Z');"
                ))?;
            }
            "unknown-export" => {
                connection.execute(
                    "INSERT INTO operations(
                         ...
                         'export-delivery-unknown','T-artifact',
                         'irreversible_external','DATA_EGRESS',
                         'export-delivery-unknown','UNKNOWN','OUTCOME_UNKNOWN',?1,
            ...
            "blob-durability" => {
                connection.execute(
                    "UPDATE artifact_blobs SET durability_state='CORRUPT'
                     WHERE content_hash=(SELECT content_hash FROM artifacts WHERE artifact_id=?1)",
                    [artifact_id],
                )?;
            }
            _ => unreachable!(),
    ...
    fn reader_delivery_transaction_rechecks_recovery_and_manager_lease() {
        for scenario in [
            ...
            let artifact = manager
                .import_artifact(
                    &import_request(),
                    &mut Cursor::new(format!("delivery {scenario}").as_bytes()),
                )
                .unwrap();
            let (binding_id, _) = install_one_shot_binding(
                &manager,
            ...
            let program_hash = allocation("unused").semantic_program_hash;
            let registry_id = format!("registry-delivery-{scenario}");
            let artifact_id = artifact.artifact_id.clone();
            READER_DELIVERY_BEFORE_TRANSACTION_TEST_HOOK.with(|hook| {
                *hook.borrow_mut() = Some(Box::new(move || {
                    mutate_reader_delivery_boundary(
                        &database,
                        &scenario_for_hook,
                        &artifact_id,
                        &program_hash,
