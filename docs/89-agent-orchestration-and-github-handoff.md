...
For authority, revocation, durable state, and retained-handle changes, prepare a compact evidence map before the first full gate. Name the public admission path, the later point-of-use path, the durable state transition, and an adversarial test for each claimed denial. A direct helper test is useful but does not establish a public API behavior. Have the test auditor identify missing paths before the expensive exact-head platform gates.

For authority work, hold schema-only groundwork in the draft workstream until a coordinator admission path and a later public use path exercise it. Focused migration tests are the right feedback during this phase; the complete platform gates and GitHub-native review belong on a frozen executable slice. Ship a migration separately only if its standalone behavior and recovery contract justify that review. State clearly that dormant schema cannot prove authorization semantics.

After two successive pushed heads produce different substantive findings within one trust boundary, use `astra_deep_review` for one high-reasoning design challenge before another narrow patch. Classify each finding as introduced by the repair, previously latent, or unresolved; use that evidence to decide whether to refactor the boundary or split a large PR into independently reviewable increments. Do not split away acceptance criteria or bypass the issue's final integrated gate. Keep the current Luna/Sol/Astra routing unless measured results justify a change.
